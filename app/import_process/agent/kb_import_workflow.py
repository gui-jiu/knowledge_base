...
class KBImportWorkflow:
    ...
    def run(self, initial_state: ImportGraphState, stream: bool = False):
        """
        统一执行入口，支持切换invoke/stream
        :param initial_state: 初始状态对象
        :param stream: 是否是流式输出
        :return: 执行完成后的状态对象（或生成器）
        """
        if not self._compiled_app:
            self.compile()
        if stream:
    ...
    @classmethod
    def create_and_run(cls, initial_state: ImportGraphState, stream: bool = False):
        """快捷方法：创建工作流实例并立即执行"""
        workflow = cls()
...
if __name__ == "__main__":
    ...
    init_state = create_default_state(
        task_id="task_demo",
        local_file_path=str(PROJECT_ROOT / "doc" / "万用表的使用.pdf"),
